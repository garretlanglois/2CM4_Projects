...
TIME 0 TO 1500  halt (t>tfuel2) and (p=0)   { if time dependent }
MONITORS         { show progress }
PLOTS            { save result displays }
	for t = 0 by endtime/100 to endtime
	history(p) at (0,0)
	history(h, v, acc) at (0,0, 0) Export Format '#t#b#1#b#2' file = 'DA1test.txt'
SUMMARY
	report eval (h, 0, 0)
	report eval (v, 0, 0)
...
lowlim = 500
highlim = 30000
fuel = np.linspace(lowlim,highlim,50)
print(fuel)
...
def bestfuel(fuelrange):
    ...
    for fuelamt in fuelrange:
        ...
        if (finalv > bestspeed).any():
            bestspeed = finalv
            print(f"current best speed is {bestspeed}")
        else:
